...
def solveCNF(path="", startLevel=10):
    ...
    for newVar in range(startLevel+1, numVar+1):
        ...
        walkerDF = walkerDF.rdd.map(lambda row: Row(id=row['id'], state=updateState(clauses = clauses, idValue = row['id'], level = newVar-1, numVar = numVar))).toDF().where(col('state') != -1)
...
graphDF = solveCNF("uf50-05.cnf", 10).cache()
